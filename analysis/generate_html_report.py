...
"""
    ...
    if analysis_data:
        ...
        # Add receptor version information section
        if analysis_data.get('versions'):
            versions = analysis_data['versions']
            html_content += """
        <h3>Receptor Version Information</h3>
        <div style="display: flex; gap: 20px; margin: 20px 0;">
            <div style="flex: 1; background: #e8f5e8; padding: 15px; border-radius: 8px;">
                <h4 style="margin-top: 0; color: #2e7d2e;">AAP 2.4 Receptor Versions</h4>
            """
            
            aap24_versions = versions.get('aap24', {})
            if aap24_versions:
                html_content += "<div style='font-family: monospace; font-size: 0.9em;'>"
                for node, version_info in aap24_versions.items():
                    status_color = "#28a745" if version_info.get('service_status') == 'Running' else "#dc3545"
                    html_content += f"""
                    <div style="margin: 8px 0; padding: 8px; background: rgba(255,255,255,0.5); border-radius: 4px;">
                        <strong>{node}:</strong><br>
                        <span style="color: #6c757d;">Version:</span> {version_info.get('version', 'Unknown')}<br>
                        <span style="color: #6c757d;">Status:</span> <span style="color: {status_color}; font-weight: bold;">{version_info.get('service_status', 'Unknown')}</span><br>
                        <span style="color: #6c757d;">Processes:</span> {'✓' if version_info.get('processes_found') else '✗'}
                    </div>
                    """
                html_content += "</div>"
            else:
                html_content += "<p>No version information collected</p>"
            
            html_content += """
            </div>
            <div style="flex: 1; background: #ffe8e8; padding: 15px; border-radius: 8px;">
                <h4 style="margin-top: 0; color: #d63384;">AAP 2.5 Receptor Versions</h4>
            """
            
            aap25_versions = versions.get('aap25', {})
            if aap25_versions:
                html_content += "<div style='font-family: monospace; font-size: 0.9em;'>"
                for node, version_info in aap25_versions.items():
                    status_color = "#28a745" if version_info.get('service_status') == 'Running' else "#dc3545"
                    html_content += f"""
                    <div style="margin: 8px 0; padding: 8px; background: rgba(255,255,255,0.5); border-radius: 4px;">
                        <strong>{node}:</strong><br>
        ...
        if comparison:
            ...
            html_content += f"""
            <div class="alert alert-info">
                <h4 style="margin-top: 0;">Memory Difference Analysis</h4>
        ...
        aap24_processes = aap24.get('process_analysis', {}).get('top_memory_consumers', {})
        if aap24_processes:
            for i, (process, data) in enumerate(list(aap24_processes.items())[:10]):
                avg_mb = data.get('avg_rss', 0) / 1024
                process_short = process[:100] + '...' if len(process) > 100 else process
                html_content += f"""
                        <tr>
        ...
        aap25_processes = aap25.get('process_analysis', {}).get('top_memory_consumers', {})
        if aap25_processes:
            for i, (process, data) in enumerate(list(aap25_processes.items())[:10]):
                avg_mb = data.get('avg_rss', 0) / 1024
                process_short = process[:100] + '...' if len(process) > 100 else process
                html_content += f"""
                        <tr>
